...
function PartnerDashboard() {
  const demandes = MOCK_DEMANDES.filter((d) => d.partenaire === "Orange Sénégal");
  ...
  return (
    <PartnerShell>
      <PageHeader
        ...
        description="Vue d'ensemble de vos demandes et candidats proposés."
        action={
          <Button asChild>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Demandes actives" value={actives} icon={FileText} hint={`${demandes.length} demandes au total`} />
        <StatCard label="Candidats proposés" value={candidatsProposes} icon={Users} hint="Sur 30 derniers jours" />
          ...
          <table className="w-full text-sm">
            <thead className="text-left text-xs uppercase tracking-wider text-ink-soft">
              <tr className="border-b border-border">
                ...
                <th className="py-3">Postes</th>
                <th className="py-3">Urgence</th>
            ...
            <tbody>
              {demandes.map((d) => (
                <tr key={d.id} className="border-b border-border/60">
                  ...
                  <td className="py-3">
                    <StatusBadge tone={d.urgence === "Haute" ? "danger" : d.urgence === "Moyenne" ? "warning" : "default"}>{d.urgence}</StatusBadge>
                  </td>
