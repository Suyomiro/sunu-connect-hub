...
function PartnerDashboard() {
  ...
  const allDemandes = useStore((s) => s.demandes);
  const demandes = useMemo(
    () => allDemandes.filter((d) => d.partenaire === company),
    [allDemandes, company],
  );
  ...
  return (
    <PartnerShell>
      <PageHeader
        ...
        description="Vue d'ensemble de vos demandes."
        action={
          <Button asChild>
            ...
            <tbody>
              ...
              {demandes.map((d) => (
                <tr key={d.id} className="border-b border-border/60">
                  ...
                  <td className="py-3">
                    <StatusBadge tone={statutTone(d.statut)}>{d.statut}</StatusBadge>
                  </td>
                </tr>
              ))}
            </tbody>
