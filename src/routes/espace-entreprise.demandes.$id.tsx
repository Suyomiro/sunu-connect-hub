...
export const Route = createFileRoute("/espace-entreprise/demandes/$id")({
  ...
  notFoundComponent: () => (
    <PartnerShell>
      <p className="text-sm text-ink-soft">Demande introuvable.</p>
    </PartnerShell>
  ),
});
...
function DemandeDetail() {
  ...
  const d = MOCK_DEMANDES.find((x) => x.id === id);
  if (!d) throw notFound();

  const proposed = MOCK_CANDIDATS.slice(0, 4);
  const tone = (s: string) =>
  ...
  return (
    <PartnerShell>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-3 rounded-2xl border border-border bg-card p-6 shadow-soft lg:col-span-1">
