...
function DemandeDetail() {
  ...
  const proposed = useStore((s) => s.candidats.slice(0, 4));
  if (!d) throw notFound();
  ...
  return (
    <PartnerShell>
      <Link to="/espace-entreprise/demandes" className="mb-4 inline-flex items-center gap-1 text-sm text-ink-soft hover:text-primary">
        <ArrowLeft className="h-4 w-4" /> Retour
      </Link>
      <PageHeader title={d.poste} description={`Référence ${d.reference} · Créée le ${new Date(d.dateCreation).toLocaleDateString("fr-FR")}`}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-3 rounded-2xl border border-border bg-card p-6 shadow-soft">
          <h3 className="font-display text-sm font-bold uppercase tracking-wider text-ink-soft">Détails du poste</h3>
          ...
          <div className="pt-4 border-t border-border">
            ...
            <Select value={d.statut} onValueChange={(v) => { demandesApi.update(d.id, { statut: v as Statut }); toast.success("Statut mis à jour"); }}>
              ...
              <SelectContent>
                {(["Nouvelle", "En traitement", "Shortlist envoyée", "Pourvue", "Clôturée"] as Statut[]).map((s) => (
          ...
          <div className="mt-4 divide-y divide-border">
            {proposed.map((c) => (
              <div key={c.id} className="flex items-center justify-between py-4">
                <div>
                  <p className="font-semibold text-ink">{c.prenom} {c.nom}</p>
                  <p className="text-sm text-ink-soft">{c.poste} · {c.experience} · {c.langues.join(", ")}</p>
                </div>
...
function Row({ icon: Icon, label, value }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string }) {
  return (
    <div className="flex items-start gap-3 py-1.5">
      ...
      <div className="text-sm"><p className="text-ink-soft">{label}</p><p className="font-medium text-ink">{value}</p></div>
    </div>
  );
}
