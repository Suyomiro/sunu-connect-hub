...
function DemandesPage() {
  ...
  const all = useStore((s) => s.demandes.filter((d) => d.partenaire === company));
  const list = all.filter((d) =>
    (statut === "all" || d.statut === statut) &&
    (q === "" || d.poste.toLowerCase().includes(q.toLowerCase()) || d.reference.toLowerCase().includes(q.toLowerCase())),
  );
  const tone = (s: string) => s === "Pourvue" ? "success" : s === "Nouvelle" ? "info" : s === "Clôturée" ? "default" : "warning";

  return (
    <PartnerShell>
      <PageHeader title="Mes demandes de recrutement" description="Suivez l'avancement de chaque demande."
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row">
        <div className="relative flex-1">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Rechercher par poste ou référence" className="pl-9" />
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-soft">
        <table className="w-full text-sm">
          <thead className="bg-secondary/60 text-left text-xs uppercase tracking-wider text-ink-soft">
            <tr>
              <th className="px-4 py-3">Référence</th><th className="px-4 py-3">Poste</th><th className="px-4 py-3">Postes</th>
              <th className="px-4 py-3">Urgence</th><th className="px-4 py-3">Statut</th><th className="px-4 py-3">Créée le</th><th className="px-4 py-3"></th>
            </tr>
          </thead>
          <tbody>
            {list.map((d) => (
              <tr key={d.id} className="border-t border-border/60 hover:bg-secondary/30">
                ...
                <td className="px-4 py-3 text-ink-soft">{new Date(d.dateCreation).toLocaleDateString("fr-FR")}</td>
                <td className="px-4 py-3 text-right">
                  <div className="flex justify-end gap-1">
                    <Link to="/espace-entreprise/demandes/$id" params={{ id: d.id }} className="text-sm font-medium text-primary hover:underline">Détails →</Link>
                    <Button size="icon" variant="ghost" onClick={() => { if (confirm("Supprimer cette demande ?")) { demandesApi.remove(d.id); toast.success("Demande supprimée"); } }}>
            ...
            {list.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-12 text-center text-sm text-ink-soft">Aucune demande. <Link to="/espace-entreprise/demandes/nouvelle" className="text-primary hover:underline">Créer la première</Link></td></tr>
            )}
          </tbody>
        </table>
      </div>
